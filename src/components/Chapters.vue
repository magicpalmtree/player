<template>
    <div class="_wrapper" :class="{'_show':chaptersIn }">
        <div class="_close" @click="handleClose">
...
export default {
    ...
    setup() {
        ...
        const { nodes } = useNode();
        const { toggleChapters } = useControlBar();
        const { setPlaying } = usePlayer();
        return {
            project, nodes, toggleChapters, setPlaying
        };
    },
    computed: {
        // ...mapGetters({
        ...
        // }),
        showChapters() {
            const { chaptersOpen } = useControlBar();
            return chaptersOpen.value;
        },

        chapters() {
            if(!this.project.chapter_items) return [];
